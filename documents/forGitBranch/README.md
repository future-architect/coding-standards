...
![リベース](merge_strategy_develop_to_feature_merge.drawio.png)
...
マージによる変更の取り込みを行う場合、メインの開発ブランチの変更を取り込むたびに機能ブランチにマージコミットが作成される。
この不要なマージコミットは、履歴を複雑にし、レビューアの混乱を招く可能性があるため、リベースにより完全に直線的な美しい履歴を作成する。

#### 注意点

マージによる変更の取り込みが既存のブランチを変更しないのに対し、リベースは全く新しい（元のコミットIDとは別のコミットIDで）コミットを作成する。
リベースを用いる場合は、次の3点に注意されたい。
...
GitのRerereを有効化する（`rerere.enabled true`）ことでコンフリクトの解消を記録し、繰り返しの操作を自動化することが望ましい。
