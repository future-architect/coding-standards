...
ブランチ管理戦略に関わらず、大半のケースにおいて、メインの開発ブランチとそこから作成される個々の機能ブランチが存在する。
...
複数人で同時並行的に開発が進む場合メインの開発ブランチは頻繁にアップデートが行われるため、開発者は自身の機能ブランチに対して、最新の開発ブランチを定期的に取り込むことが望まれる。
機能ブランチに対して開発ブランチの変更を取り込む方法は「マージ」による方法と「リベース」による方法の大きく2つが考えられる。
...
メモ
* feature -> developへの取り込みは「Squash & Merge」を推奨で異論なさそう
* featureに最新developをどう取り込むかは議論の余地あり
  * コンフリクトの解消はMergeでもRebaseでも発生する
  * Rebaseする場合、featureブランチのコミット履歴が多いと、MAXでコミット履歴分、コンフリクト解消が必要になるのが手間
  * すでにPush済のRebaseする場合、Force Pushが必要。
  * GitLabでForce Pushするとレビューコメントの紐付けが壊れる。
  * Rebaseしてくれた方がみやすい。 Merge CommitやRebase & Mergeが発生することを見越すと、featureブランチのログも綺麗に作っておきたい。
-> 落とし所。
  * 個人的にはRebase推奨にしたい。ただ、GitLab使う場合や開発者のレベルによっては、最終的に「Squash & Merge」をすることを前提としてMerge採用してもいいよ、くらいか。

* develop -> stagingや Staging -> mainなどは、Merge Commit一択
* 複数developブランチが存在している場合のも検討したい。Mergeじゃないと厳しそう。
